...
# local imports
import rsl_rl_cli_args as cli_args  # isort: skip
...
def main():
    ...
    np.save(os.path.join(log_dir, name_prefix + "obs_dataset.npy"), obs_numpy)
    np.save(os.path.join(log_dir, name_prefix + "states_dataset.npy"), states_numpy)
    np.save(
        os.path.join(log_dir, name_prefix + "constraints_dataset.npy"),
        constraints_numpy,
    )
    np.save(
